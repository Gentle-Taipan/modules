...
resource "aws_eip" "nat" {
  ...
  domain = "vpc"
  tags   = merge(var.tags, { Name = "${var.env}-Nat-Gateway-${count.index + 1}" })
}
...
resource "aws_subnet" "private_subnets" {
  ...
  availability_zone = data.aws_availability_zones.available.names[count.index]
  tags              = merge(var.tags, { Name = "${var.env}-Private-Subnets-${count.index + 1}" })
}
...
resource "aws_route_table_association" "prvate-routes" {
